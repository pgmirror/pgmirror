...
those as normal but that is just silly. Pgmirror is a bit more useful than a simple CRUD generator: place @ annotations in 
comments on tables and columns to get additional functionality.
...
Postgresql reports all view columns as nullable which is unfortunate because that makes all
...
**The only supported table structure is one with a single primary key. The generated repositories won't work for tables without
a PK or tables with composite PKs.**
...
object UserListViewDoobieRepository {
  def listFiltered(
    ...
    val createdFilter = created.map(v => fr"created >= $v")
    val q: Fragment = 
