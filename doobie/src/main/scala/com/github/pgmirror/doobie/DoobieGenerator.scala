...
class DoobieGenerator(settings: Settings) extends Generator(settings) {
  ...
  override def generateForTable(
    ...
    List(
      GeneratedFile(
        modelPath,
        table.className + ".scala",
        generateTableClass(settings, table),
      ),
      GeneratedFile(
        repositoryPath,
        table.className + "Repository.scala",
        repository,
      ),
    )
  }
  ...
  private def columnDefault(p: String) = p match {
    ...
    case t =>
      throw new UnsupportedOperationException(
        s"Unsupported primary key type: $t",
      )
  }

  private def generateTableClass(
      settings: Settings,
      table: TableLike,
  ): String = {
  ...
  private def generateTableRepository(
      settings: Settings,
      table: TableLike,
  ): String = {
    ...
    val finds = table.columns.filter(c => c.annotations.contains(Find)).map {
      c =>
    ...
    val findOnes =
      table.columns.filter(c => c.annotations.contains(FindOne)).map { c =>
    ...
    s"""package ${tablePackage(settings.rootPackage, table.schemaName)}.repository
       ...
       |  val tableColumns = List(${table.columns
         .map(_.columnNameQuoted)
         .mkString(",")})
       |
  ...
  private def generateViewRepository(
      settings: Settings,
      view: TableLike,
  ): String = {
    ...
    val filters: List[(String, String, String)] = for {
      ...
      val (paramName, paramNameVal) = ann match {
        ...
        case _ =>
          throw new IllegalArgumentException("Only filter annotations allowed!")
      }
