...
import com.github.pgmirror.core.model.gatherer.{
  PgColumns,
  PgForeignKeys,
  PgTables,
}
...
class DatabaseSchemaGatherer(settings: Settings) {
  ...
  private def columnAnnotations(
      comment: Option[String],
  ): Set[ColumnAnnotation] =
    ColumnAnnotation.values
      .filter(_.regex.findAllIn(comment.getOrElse("")).nonEmpty)
      .toSet
  ...
  def gatherDatabase: Either[List[Throwable], Database] = {
    ...
    val pgForeignKeys =
      runStatement(
        database.prepareStatement(PgForeignKeys.sql),
        PgForeignKeys.fromResultSet,
      ).filter(f => settings.schemaFilter.matcher(f.tableSchema).matches())

    val rawTables =
      if (settings.schemas.nonEmpty) {
        val ps =
          database.prepareStatement(PgTables.sqlSchemaInList(settings.schemas))
        ...
        runStatement(
          database.prepareStatement(PgTables.sql),
          PgTables.fromResultSet,
        )
      }
    ...
    val pgColumns = runStatement(
      database.prepareStatement(PgColumns.sql),
      PgColumns.fromResultSet,
    ).filter(t =>
    ...
    val columns = pgColumns.map { pgc =>
      val schema =
        if (pgc.udtSchema == settings.defaultSchema) "" else pgc.udtSchema
      val tableSchema =
        if (pgc.tableSchema == settings.defaultSchema) "" else pgc.tableSchema
      ...
      SqlTypes
        ...
        .map { dt =>
          ...
          Column(
            ...
            isNullable = pgc.isNullable && !annotations
              .contains(ColumnAnnotation.NotNull),
    ...
    if (allErrors.nonEmpty) {
      ...
      val tablesAndViews =
        pgTables.map { pgt =>
          val tableSchema =
            if (pgt.tableSchema == settings.defaultSchema) ""
            else pgt.tableSchema
          TableLike(
            tableType =
      ...
      val tableFKs =
        for {
          fgk <- pgForeignKeys
          fkTableSchema = if (fgk.tableSchema == settings.defaultSchema) ""
          else fgk.tableSchema
          fkForeignTableSchema = if (fgk.foreignTableSchema == settings.defaultSchema)
            ""
          else fgk.foreignTableSchema
          table <- tables
                    .find(t =>
                      t.schemaName == fkTableSchema && t.name == fgk.tableName,
                    )
                    .toList
          col <- table.columns.find(_.name == fgk.columnName).toList
          foreignTable <- tables
                           .find(t =>
                             t.schemaName == fkForeignTableSchema && t.name == fgk.foreignTableName,
                           )
                           .toList
          foreignCol <- foreignTable.columns
                         .find(_.name == fgk.foreignColumnName)
                         .toList
        } yield ForeignKey(
          table,
          col,
